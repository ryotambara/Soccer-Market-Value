"""
pipeline/clean_2024.py

Normalises positions and nationalities, creates dummy variables,
and saves cleaned data for the 2024-25 season model.

Position priority:
  1. TM position (position_tm) — same format as 2025-26, uses POSITION_MAP
  2. WhoScored position group (position_group_ws) — fallback, already mapped

Run: python pipeline/clean_2024.py
"""
...
MERGED_PATH = os.path.join(BASE, "..", "data", "processed", "2024-25", "merged.csv")
OUT_PATH    = os.path.join(BASE, "..", "data", "processed", "2024-25", "cleaned.csv")

# ---------------------------------------------------------------------------
# Position mapping — Transfermarkt labels → standardised groups
# Centre-Back is baseline (no dummy created).
# Identical to clean.py.
# ---------------------------------------------------------------------------
POSITION_MAP = {
...
# ---------------------------------------------------------------------------
# Nationality mapping — identical to clean.py
# ---------------------------------------------------------------------------
AFRICAN_NATIONS = {
...
def resolve_position_group(row: pd.Series) -> str:
    """
    Use TM position if available (authoritative), else fall back to
    the WhoScored-mapped position_group_ws from Football Data.
    """
...
def main():
    ...
    print("=" * 60)
    print("Pipeline 2024-25 — Clean & Normalise")
    print("=" * 60)
    ...
    if "contract_months_remaining" in df.columns:
        ...
        # If median is NaN (all values missing), fall back to 18 months
    ...
    # Compute xg_diff from goals - xg if WhoScored didn't supply it
