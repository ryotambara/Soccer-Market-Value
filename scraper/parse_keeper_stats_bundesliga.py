"""
scraper/parse_keeper_stats_bundesliga.py

Parses goalkeeper stats CSV for the Bundesliga 2025-26 season,
fuzzy-matches players into the existing features file, and adds
GK-specific interaction columns.

Input files:
  data/raw/bundesliga_keepers_2025-26.csv

Output: updates in-place
  data/processed/bundesliga/features.csv

Run: python scraper/parse_keeper_stats_bundesliga.py
"""
...
SEASONS = [
    {
        "label":         "2025-26",
        "raw_path":      os.path.join(BASE_DIR, "data", "raw", "Bundesliga", "2025-26", "keepers.csv"),
        "features_path": os.path.join(BASE_DIR, "data", "processed", "bundesliga", "2025-26", "features.csv"),
    },
]
...
def process_season(season: dict) -> None:
    ...
    # Verification: Neuer / ter Stegen
    for name in ("Neuer", "ter Stegen", "Trapp"):
        row = keepers[keepers["player_name"].str.contains(name, na=False)]
        if not row.empty:
            print(f"\n  {name} stats:")
            print(row[["player_name", "club", "save_pct", "cs_per_90",
...
def main() -> None:
    print("=" * 60)
    print("Bundesliga Goalkeeper Stats Parser")
    print("=" * 60)
    ...
    print(f"\n{'=' * 60}")
    print("Done. Run regression to use the new GK features.")
    print("  python model/regression_bundesliga.py")
    print("=" * 60)
